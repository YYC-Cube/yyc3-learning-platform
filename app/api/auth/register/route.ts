...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 检查邮箱是否已存在
    const existingEmails = await query('SELECT id FROM learning_users WHERE email = $1', [email]);
    if (existingEmails.length > 0) {
      return applySecurityHeadersToNextResponse(
        NextResponse.json({ error: '该邮箱已被注册' }, { status: 409 })
      );
    }

    // 检查用户名是否已存在
    const existingUsernames = await query('SELECT id FROM learning_users WHERE username = $1', [
      username,
    ]);
    if (existingUsernames.length > 0) {
      return applySecurityHeadersToNextResponse(
        NextResponse.json({ error: '该用户名已被使用' }, { status: 409 })
      );
    }
