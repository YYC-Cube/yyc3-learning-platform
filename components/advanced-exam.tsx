/**
 * @fileoverview UI组件 · advanced-exam.tsx
...
export function AdvancedExam({ examType, timeLimit = 90, onComplete }: AdvancedExamProps) {
  ...
  const handleSubmitExam = useCallback(() => {
    ...
    examQuestions.forEach((question) => {
      ...
      if (question.type === 'definition') {
        const keywords = (question.referenceAnswer || '')
          .split(/[，,、；;。]/)
          .map((k) => k.trim());
        ...
        const requiredPoints = (question.referenceAnswer || '')
          .split(/[，,、；;。]/)
          .map((p) => p.trim());
        const matchedPoints = requiredPoints.filter((p) => userPoints.some((up) => up.includes(p)));
        const pointsEarned = Math.round(
          (matchedPoints.length / requiredPoints.length) * question.points
        );
        earnedPoints += pointsEarned;
        ...
        const requiredKeywords = (question.referenceAnswer || '')
          .split(/[，,、；;。]/)
          .map((k) => k.trim());
        ...
        const pointsEarned = Math.round(
          (matchedKeywords.length / requiredKeywords.length) * question.points
        );
        earnedPoints += pointsEarned;

        if (pointsEarned < question.points) {
          feedback[question.id] =
            `应用要点匹配度: ${matchedKeywords.length}/${requiredKeywords.length}`;
        }
      }
    });
  ...
  useEffect(() => {
    ...
    switch (examType) {
      case 'definition':
        questions = generateAdvancedExamPaper({
          definitionCount: 10,
          comparisonCount: 0,
          applicationCount: 0,
        });
