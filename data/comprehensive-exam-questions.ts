...
`,
  ...
  // 按应用场景分类题目
  {
    ...
    correctAnswers: [0, 1, 3],
    explanation:
      '写作助手、机器翻译和聊天机器人都涉及文本的生成和处理。图像识别属于计算机视觉任务，不是文本生成。',
    points: 3,
...
`,
  ...
  // 按规模大小分类题目
  {
    ...
    correctAnswers: [1],
    explanation:
      'GPT-3拥有1750亿参数，属于大型模型。BERT-base约1.1亿参数，MobileBERT和DistilBERT都是轻量化模型。',
    points: 2,
...
`,
  ...
  // 按开源状态分类题目
  {
    ...
    correctAnswers: [0, 1],
    explanation:
      'Meta Llama系列和HuggingFace Transformers都是开源模型。GPT-4和百度文心一言是商业化的闭源模型。',
    points: 3,
...
`,
  ...
  },
  {
    ...
    correctAnswers: [2],
    explanation:
      '第三代AI模型的核心突破是Transformer架构，它基于自注意力机制，支持并行化训练，显著提升了模型性能。',
    points: 2,
...
export const extendedSingleChoiceQuestions: ComprehensiveExamQuestion[] = [
  ...modelClassificationQuestions.filter((q) => q.type === 'single'),
  {
    ...
    correctAnswers: [2],
    explanation:
      'LoRA通过低秩分解的方式，在微调大语言模型时只更新少量参数，实现参数高效的微调，大大降低了微调成本。',
    points: 2,
...
export function generateClassificationExam(config: {
  ...
  if (config.classifications.length > 0) {
    availableQuestions = availableQuestions.filter((q) =>
      config.classifications.some((c) => q.category.includes(c))
    );
  }
