...
export function generateExamPaper(config: {
  ...
  const singleQuestions = availableQuestions
    .filter((q) => q.type === 'single')
    .slice(0, config.singleCount);

  const multipleQuestions = availableQuestions
    .filter((q) => q.type === 'multiple')
    .slice(0, config.multipleCount);

  const essayQuestions = availableQuestions
    .filter((q) => q.type === 'essay')
    .slice(0, config.essayCount);
