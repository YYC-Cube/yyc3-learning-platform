...
export class EnhancedLearningSystem extends EventEmitter {
  ...
  private calculateLearningImpact(experience: LearningData): number {
    ...
    const successWeight =
      experience.type === 'success' ? 1 : experience.type === 'partial' ? 0.5 : -0.5;
    const qualityWeight =
      {
        excellent: 1.0,
        good: 0.75,
    ...
    for (const domainName of applicableDomains) {
      ...
      if (existingPattern) {
        ...
        existingPattern.effectiveness =
          (existingPattern.effectiveness + this.calculateLearningImpact(experience)) / 2;
      } else {
  ...
  private async updateStrategies(experience: LearningData): Promise<void> {
    ...
    for (const domainName of applicableDomains) {
      ...
      strategy.successRate =
        (strategy.successRate * (strategy.usageCount - 1) + (success ? 1 : 0)) /
        strategy.usageCount;

      strategy.effectiveness =
        strategy.effectiveness * (1 - this.config.learningRate) +
  ...
  private async analyzeWithAI(experience: LearningData): Promise<void> {
    ...
    try {
      ...
      const request: any = {
        ...
        maxTokens: 500,
        systemPrompt:
          'You are a learning analysis system. Analyze the provided experience and extract key insights.',
        metadata: {
...
Provide 3-5 actionable insights for improving future performance in similar situations.`;
  ...
  private calculateCompetencyLevel():
    ...
    | 'expert' {
    const avgCompetency =
      Array.from(this.knowledgeDomains.values()).reduce(
        (sum, domain) => sum + domain.competency,
        0
      ) / this.knowledgeDomains.size;
  ...
  private getRecentInsights(): readonly Insight[] {
    ...
    const insights: Insight[] = recentExperiences.map((exp) => ({
      ...
      impact:
        exp.metadata.importance === 'high'
          ? ('high' as ImpactLevel)
          : exp.metadata.importance === 'medium'
  ...
  getPerformanceMetric(
    metricName: string
  ): { average: number; min: number; max: number; count: number } | undefined {
    const metrics = this.performanceMetrics.get(metricName);
