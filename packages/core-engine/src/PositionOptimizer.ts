  ...
  generatePositions(
    ...
    if (this.rules.get('nearEdge')! > 0.5) {
      candidates.push(
        ...
        {
          position: { x: context.screen.width - component.size.width - 20, y: 20 },
          source: 'rule',
          metadata: { rule: 'top-right' },
        },
        {
          position: { x: 20, y: context.screen.height - component.size.height - 20 },
          source: 'rule',
          metadata: { rule: 'bottom-left' },
        },
        {
          position: {
            x: context.screen.width - component.size.width - 20,
            y: context.screen.height - component.size.height - 20,
          },
          source: 'rule',
          metadata: { rule: 'bottom-right' },
        }
      );
    }
    ...
    candidates.push(
      {
        position: { x: context.screen.width / 2 - component.size.width / 2, y: 20 },
        source: 'rule',
        metadata: { rule: 'top-center' },
      },
      {
        position: {
          x: context.screen.width / 2 - component.size.width / 2,
          y: context.screen.height - component.size.height - 20,
        },
        source: 'rule',
        metadata: { rule: 'bottom-center' },
      },
      {
        position: { x: 20, y: context.screen.height / 2 - component.size.height / 2 },
        source: 'rule',
        metadata: { rule: 'left-center' },
      },
      {
        position: {
          x: context.screen.width - component.size.width - 20,
          y: context.screen.height / 2 - component.size.height / 2,
        },
        source: 'rule',
        metadata: { rule: 'right-center' },
      }
    );
...
export class PositionOptimizer extends EventEmitter {
  ...
  private async generateCandidates(
    ...
    if (constraints.avoidAreas) {
      const avoidBased = this.generateAvoidancePositions(
        component,
        context,
    ...
    if (context.interactionZones.length > 0) {
      const minDistance = Math.min(
        ...context.interactionZones.map((zone) =>
          this.distance(candidate.position, {
  ...
  private async scoreStability(
    ...
    if (memory) {
      const nearHistorical = memory.positions.some(
        (p) => this.distance(candidate.position, p) < 100
      );
  ...
  private async collectContext(component: UIComponent): Promise<OptimizationContext> {
    ...
    return {
      ...
      visibleElements: Array.from(
        document.querySelectorAll('[data-visible="true"]')
      ) as HTMLElement[],
  ...
  private calculateEdgeDistance(position: Position, screen: ScreenInfo): number {
    return Math.min(position.x, position.y, screen.width - position.x, screen.height - position.y);
  }
